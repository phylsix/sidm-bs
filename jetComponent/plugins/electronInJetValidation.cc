// -*- C++ -*-
//
// Package:    sidm-bs/electronInJetValidation
// Class:      electronInJetValidation
#include <algorithm>
#include <cmath>
...
#include "sidm-bs/jetComponent/interface/electronInJetValidation.h"
#include "sidm-bs/jetComponent/interface/utilities.h"

sidm::electronInJetValidation::electronInJetValidation(const edm::ParameterSet& iConfig):
...
sidm::electronInJetValidation::~electronInJetValidation()
{
...
void
sidm::electronInJetValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    using namespace edm;
    using namespace std;
    ...
    int IS_EP(-1), IN_JT(-1);  // Categorize them
    ...
    std::copy_if(patElectronPtr_.begin(), patElectronPtr_.end(), back_inserter(patEp_es),
                 [](const Ptr<pat::Electron>& p){return p->charge()<0;});
    std::copy_if(patElectronPtr_.begin(), patElectronPtr_.end(), back_inserter(patEp_ps),
                 [](const Ptr<pat::Electron>& p){return p->charge()>0;});
        ...
        if ( epPairTmp.size() == 0 ) {
            ...
            if (epPairTmp.size() == 1) {
                ...
                if (matchedNum == 2) {
                    ...
                    IS_EP = 2;         //< Both are electron-positron pairs.
                    IN_JT = 0;
                    ++event2pairs_;
                    ++event2pairsEpJt_;
                }
                else if (matchedNum == 1) { PAIR = 1; IS_EP = 1; }
                else if (matchedNum == 0) { PAIR = 0; IS_EP = 0; }
    ...
    /**
     * __ELECTRON PAIR IN JET SEARCH__
     */
    ...
    if ( PAIR==1 ) {
        ...
        for (const auto& q : genZps) {
            ...
            // Should only be ONE Zp unmatched
            theUnmatched = q;
        }
        for (const auto& j : patJetPtr_) {                                       
            pair<float, float> dR_ep_wrt_jet = theUnmatched.dRVal(j);
            if (dR_ep_wrt_jet.first <= dRusb_ && dR_ep_wrt_jet.second <= dRusb_)
                patJetPtrTmp.push_back(j);
        }


        if ( patJetPtrTmp.size() == 0 ) {
            IN_JT = 0;
            ++event1pair_; ++event1pairEp_;
        } else {
            PAIR = 2; IN_JT = 1;
            ++event2pairs_; ++event2pairsEpJt_;
            if ( patJetPtrTmp.size() == 1 ) {
                j_fake_e_ = sidm::Jet(patJetPtrTmp[0]);
                jet_properties_->Fill();
                //sidm::remove_from_collection(&patJetPtr_, patJetPtrTmp[0]);
            }
            if ( patJetPtrTmp.size() > 1 ) {
                sort(begin(patJetPtrTmp), end(patJetPtrTmp),
                        [&theUnmatched](const auto& lhs, const auto& rhs){
                        pair<float, float> lhs_dr = theUnmatched.dRVal(lhs);
                        pair<float, float> rhs_dr = theUnmatched.dRVal(rhs);
                        return ( (lhs_dr.first+lhs_dr.second)/2 < (rhs_dr.first+rhs_dr.second)/2 );});
                j_fake_e_ = sidm::Jet(patJetPtrTmp[0]);
                jet_properties_->Fill();
            }
        }
    ...
    if ( PAIR==0 ) {
        for (auto& q : genZps) {
            vector<Ptr<pat::Jet> > patJetPtrTmp{};
            for (const auto& j : patJetPtr_) {
                pair<float, float> dR_ep_wrt_jet = q.dRVal(j);
                if (dR_ep_wrt_jet.first <= dRusb_ && dR_ep_wrt_jet.second <= dRusb_)
                    patJetPtrTmp.push_back(j);
            }
            if ( patJetPtrTmp.size() == 0 ) {
                ...
                if ( patJetPtrTmp.size() == 1 ) {
                    ...
                    std::sort(begin(patJetPtrTmp), end(patJetPtrTmp),
                            [q](const auto& lhs, const auto& rhs){
                                pair<float, float> lhs_dr = q.dRVal(lhs);
                                pair<float, float> rhs_dr = q.dRVal(rhs);
                                return ((lhs_dr.first+lhs_dr.second)/2 < (rhs_dr.first+rhs_dr.second)/2);
                            });
                    j_fake_e_ = sidm::Jet(patJetPtrTmp[0]);
                    jet_properties_->Fill();
        ...
        int matchedNum = std::count_if(cbegin(genZps), cend(genZps),
                [](const auto& q){return q.matched;});
        if ( matchedNum == 0 ) {
            PAIR = 0; IN_JT = 0;
            ++event0pair_;
            ...
            PAIR = 1; IN_JT = 1;
            ++event1pair_; ++event1pairJt_;
        } else { // SHOULD BE 2
            PAIR = 2; IN_JT = 2;
            ++event2pairs_; ++event2pairsJtJt_;
        }
    }

    // ---------------ELECTRON IN JET SEARCH END---------------


    
    assert(PAIR == IS_EP + IN_JT);
...
void 
sidm::electronInJetValidation::beginJob()
{
...
void 
sidm::electronInJetValidation::endJob() 
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
sidm::electronInJetValidation::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(sidm::electronInJetValidation);
